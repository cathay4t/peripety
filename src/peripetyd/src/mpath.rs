...
fn get_mpath_info_from_blk(major_minor: &str) -> (String, String) {
    ...
    let mut holders = fs::read_dir(&sysfs_holder_dir).unwrap();
    if let Some(Ok(holder)) = holders.next() {
        let dm = holder.path();
...
fn parse_event(event: &StorageEvent, sender: &Sender<StorageEvent>) {
    match event.event_type.as_ref() {
        "DM_MPATH_PATH_FAILED" | "DM_MPATH_PATH_REINSTATED" => {
            ...
            event.dev_name = name;
            event.dev_wwid = uuid;
            event.extention.insert("blk_name".to_string(),
                                   Sysfs::major_minor_to_blk_name(&event.kdev));
            event.extention.insert("blk_major_minor".to_string(),
                                   event.kdev.clone());
